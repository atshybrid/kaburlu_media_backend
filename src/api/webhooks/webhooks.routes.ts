...
async function processWhatsappBotMessage(phone: string, text: string, mediaId: string | null = null) {
  ...
  if (!session) {
    ...
    if (isGreeting || !TRIGGER_KEYWORDS.some(k => inputLower.includes(k))) {
      ...
      if (existingProfile) {
        // Check if this person is a president / union admin
        const isAdmin = await checkIsAdminOrPresident(existingUser!.id, existingProfile.id, existingProfile.unionName || unionName || '');
        if (isAdmin) {
          await showPresidentMenu(phone, existingUser!.id, existingProfile.id, existingProfile.unionName || unionName || '', existingProfile.state || null, unionDisplayName || unionName || 'DJFW');
          return;
        }
    ...
    case 'PRESIDENT_MENU': {
      ...
      if (inputLower === 'admin_my_card') {
        ...
        if (!pdfUrl) {
          ...
          if (!card) {
            const expiry = new Date(); expiry.setFullYear(expiry.getFullYear() + 1);
            await (prisma as any).journalistCard.create({ data: { profileId, cardNumber: `JU-${Date.now()}`, expiryDate: expiry, status: 'ACTIVE' } });
          }
          const r = await generateAndUploadPressCardPdf(profileId);
          if (r.ok && r.pdfUrl) pdfUrl = r.pdfUrl;
        }
        if (pdfUrl) {
          const sent = await sendWhatsappIdCardTemplate({
            toMobileNumber: phone, pdfUrl,
            cardType: 'DJFW ID', organizationName: 'DJFW',
            documentType: 'Press ID Card', pdfFilename: `Press_ID_${profileId.slice(-8).toUpperCase()}.pdf`,
          });
          if (!sent.ok) await reply(phone, `🪪 మీ ID కార్డ్ లింక్: ${pdfUrl}`);
        } else {
    ...
    case 'AWAIT_KYC_REVIEW': {
      ...
      if (input.startsWith('kyc_approve:')) {
        ...
        if (jp?.user?.mobileNumber) {
          ...
          let pdfUrl: string | null = jp.card?.pdfUrl || null;
          if (!pdfUrl) {
            await reply(memberPhone, `⏳ మీ ID కార్డ్ తయారు చేస్తున్నాం...`);
            let card = await (prisma as any).journalistCard.findUnique({ where: { profileId: pid } }).catch(() => null);
            if (!card) {
              const exp = new Date(); exp.setFullYear(exp.getFullYear() + 1);
              await (prisma as any).journalistCard.create({ data: { profileId: pid, cardNumber: `JU-${Date.now()}`, expiryDate: exp, status: 'ACTIVE' } });
            }
            const r = await generateAndUploadPressCardPdf(pid);
            if (r.ok && r.pdfUrl) pdfUrl = r.pdfUrl;
          }
          if (pdfUrl) {
            const sent = await sendWhatsappIdCardTemplate({
              toMobileNumber: memberPhone, pdfUrl,
              cardType: 'DJFW ID', organizationName: jp.currentNewspaper || jp.organization || 'DJFW',
              documentType: 'DJFW ID Card', pdfFilename: `Press_ID_${pid.slice(-8).toUpperCase()}.pdf`,
            });
            if (!sent.ok) await reply(memberPhone, `✅ *KYC అప్రూవ్ అయింది!* మీ ID కార్డ్: ${pdfUrl}`);
          }
          await reply(memberPhone,
            `🎉 *${memberName}గారికి అభినందనలు!*\n\n` +
            `✅ మీ KYC వెరిఫికేషన్ పూర్తైంది.\n` +
            `🛡️ ఇన్సూరెన్స్ త్వరలో యాక్టివేట్ అవుతుంది.\n` +
            `• అపఘాత బీమా: ₹5 లక్షలు\n• కుటుంబ ఆరోగ్య బీమా: ₹3 లక్షలు`
          );
        }
...
async function checkIsAdminOrPresident(userId: string, profileId: string, unionName: string): Promise<boolean> {
  const unionAdmin = await (prisma as any).journalistUnionAdmin.findFirst({
    where: { userId, unionName },
  }).catch(() => null);
  if (unionAdmin) return true;
...
async function showPresidentMenu(phone: string, userId: string, profileId: string, unionName: string, state: string | null, displayName: string) {
  ...
  const postTitle = postHolder?.post?.title || 'Admin';
  ...
  await replyButtons(phone,
    ...
    `మీరు ఏమి చేయాలనుకుంటున్నారు?`,
    [
      { id: 'admin_my_card', title: '🪪 నా ID కార్డ్ డౌన్‌లోడ్' },
      { id: 'admin_kyc_list', title: `⏳ KYC అప్రూవల్ (${pending})` },
    ]
  );
...
async function showPresidentMenuByStep(phone: string, data: Record<string, any>) {
  ...
  await replyButtons(phone,
    ...
    [
      { id: 'admin_my_card', title: '🪪 నా ID కార్డ్' },
      { id: 'admin_kyc_list', title: `⏳ KYC అప్రూవల్ (${pending})` },
    ]
  );
