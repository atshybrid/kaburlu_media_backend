    ...
    case 'CONFIRM': {
      ...
      try {
        ...
        if (user) {
          ...
          if (data.reporterId) {
            // Tenant reporter joining union — create JournalistProfile with their existing photo and keep approval pending
            ...
            await reply(phone,
              `✅ మీ ప్రొఫైల్ సృష్టించబడింది.

` +
              `📌 మీరు *${unionCardName}* సభ్యులుగా చేరారు, కానీ ID కార్డ్ ఇప్పుడు *అడ్మిన్ అప్రూవల్ కోసం పెండింగ్* లో ఉంది.
` +
              `📩 అడ్మిన్ అప్రూవ్ చేసిన తర్వాత మీ ID కార్డ్ WhatsApp లో పంపబడుతుంది.
` +
              `
` +
              `🛡️ ఇప్పుడు ఆధార్, PAN, నామినీ వివరాలు నమోదు చేయాలా?`
            );
            await replyButtons(phone,
              `🛡️ *ఇన్స్యూరన్సు దరఖాస్తు*`,
              [{ id: 'insurance_yes', title: '🛡️ అవును, దరఖాస్తు చేయి' }, { id: 'insurance_no', title: '❌ వద్దు, తర్వాత' }]
            );
            await (prisma as any).whatsappBotSession.update({
              where: { phone },
        ...
        if (jp?.user?.mobileNumber) {
          ...
          if (card) {
            await reply(memberPhone,
              ...
              `✅ మీ KYC అప్రూవ్ అయింది. మీ ID కార్డ్ ఇప్పుడు *సిద్ధంగా ఉంది*, కానీ ఇది వెంటనే పంపకపోవచ్చు.\n` +
              `📩 24 గంటల లోపు లేదా మీరు తిరిగి *hi* / *DJFW* అని పంపినప్పుడు మీ ID కార్డ్ అందుతుంది.\n` +
