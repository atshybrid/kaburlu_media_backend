...
async function retryAnakapalli() {
  try {
    const filePath = path.join(process.cwd(), 'location', 'Andhrapradesh_mandals', 'Anakapalli.json');
    ...
    if (!district) {
      ...
      console.log('✅ Created Anakapalli district');
    ...
    console.log(`\n🚀 Completing Anakapalli (${data.mandals.length} mandals)...\n`);
    ...
    for (let i = 0; i < data.mandals.length; i++) {
      ...
      try {
        ...
        console.log(`  ${(i + 1).toString().padStart(2)}/21 ✅ ${mandalData.name_en}`);
        
        // Small delay
        await new Promise(resolve => setTimeout(resolve, 150));
      } catch (error: any) {
        console.error(`  ${(i + 1).toString().padStart(2)}/21 ❌ ${mandalData.name_en}: ${error.message}`);
      }
    }
